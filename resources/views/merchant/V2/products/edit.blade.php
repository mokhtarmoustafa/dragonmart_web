...
<div class="card card-custom">
  ...
  <div class="card-header">
    ...
    <div class="card-title">
      <h3 class="card-label">{{trans(lang_app_site().'.CP.Edit product')}}
      </div>

      <div class="card-toolbar">
        <a href="{{url('merchant/product/create')}}" class="btn btn-circle btn-primary add-category-mdl" >
    ...
    <div class="card-body">
      ...
      {!! Form::open(['method'=>'PUT','class'=>'form-horizontal form-bordered form-row-stripped', 'url'=>url($EditView),'id'=>'productEdit']) !!}
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item">
          <a class="nav-link active" id="general-tab" data-toggle="tab" href="#general">
            ...
            <span class="nav-text">{{trans(lang_app_site().'.CP.General')}}</span>
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" id="customizations-tab" data-toggle="tab" href="#customizations" aria-controls="profile">
            ...
            <span class="nav-text">{{trans(lang_app_site().'.CP.Customizations')}}</span>
          </a>
        </li>
      </ul>
            ...
            {{--<form method="POST" action="#" class="form-horizontal form-bordered form-row-stripped">--}}
              ...
              <div class="form-group row">
                <div class="col-lg-4">
                  ...
                  <input id="name" class="form-control" name="name" type="text" placeholder="Product name" value="{{$product->name}}">
                </div>
                <div class="col-lg-4">
                  <label>{{trans(lang_app_site().'.CP.Price')}}</label>
                  <input id="price" class="form-control" name="price" type="number" step=".01" placeholder="Price" value="{{$product->price}}">
                </div>
                <div class="col-lg-4">
                  <label>{{trans(lang_app_site().'.CP.Quantity')}}</label>
                  <input id="original_quantity" class="form-control" name="original_quantity" type="number" placeholder="Quantity" value="{{$product->original_quantity}}">
                </div>
              </div>
                ...
                <div class="col-lg-4">
                  ...
                  <div class="input-group">
                    <div class="input-group-prepend">
                      <span class="input-group-text">
                        <label class="checkbox checkbox-inline checkbox-success">
                          <input class="is_sponsor" @if($product->is_sponsor) checked data-value="1" @else data-value="0" @endif type="checkbox" id="checkbox2">
                          <span></span>
                        </label>
                      </span>
                    </div>
                    <input id="sponsor_duration" class="form-control" name="sponsor_duration" type="number" placeholder="Number of days" @if($product->is_sponsor) value="Number of days"  @else disabled @endif >
                    <div class="input-group-append">
                      <span class="input-group-text">{{trans(lang_app_site().'.CP.Days')}}</span>
                    </div>
                  </div>
...
foreach($product->custom as $opt){
                ...
                <div class="option-section w-100" id="option_section_0">
                  <div class="row padding-sm">
                    <div class="col-md-3">
                      <div class="form-group">
                        <div class="input-group">
                          ...
                          <select id="option_group_type_" type="text" class="form-control option_group_type product_price_ option_group_cls" onchange="ChangeType(null , this)" name="custom[{{$n}}][type]">
                            <option value="text"  @if($opt['type'] == 'text') selected @endif >text</option>
                            <option value="color" @if($opt['type'] == 'color') selected @endif >color</option>
                          </select>
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        <div class="input-group">
                          <span class="input-group-addon input-group-addon-small"><i class="sicon-type-square"></i></span>
                          <input id="option_group_name_" type="text" class="form-control option_group_name product_price_ option_group_cls" placeholder="مسمى الخيار (مثل اللون، المقاس، ..)" value="{{ $opt['name'] }}" name="custom[{{$n}}][name]">
                        </div>
                      </div>
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        <div class="input-group">
                          <span class="input-group-addon input-group-addon-small"><i class="sicon-type-square"></i></span>
                          <input id="option_group_min_" type="number" class="form-control option_group_min product_price_ option_group_cls" placeholder="الحد الأدني للإختيار" value="{{$opt['min']}}" name="custom[{{$n}}][min]" min="0">
                        </div>
                      </div>
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        <div class="input-group">
                          <span class="input-group-addon input-group-addon-small"><i class="sicon-type-square"></i></span>
                          <input id="option_group_max_" type="number" class="form-control option_group_max product_price_ option_group_cls" placeholder="الحد الأعلى للإختيار" value="{{$opt['max']}}" name="custom[{{$n}}][max]" min="1">
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="option-options w-100" id="option-options_{{$n}}">
                    <?php
                                  $i = 0 ;
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          <div class="input-group">
                            <input id="product_name_0" type="{{$opt['type']}}" class="form-control option_details_name product_name_0 option_values_cls" placeholder="خيار {{$n}}" value="{{$option['details_name']}}" name="custom[{{$n}}][option][{{$i}}][details_name]">
                          </div>
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-group">
                          <div class="input-group">
                            ...
                            <input id="product_price_0" type="text" class="form-control product_price_0  _parseArabicNumbers" placeholder="السعر الإضافي" value="{{$option['details_price']}}" name="custom[{{$n}}][option][{{$i}}][details_price]">
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <button type="button" id="add_option" class="btn btn-info mt-1" onclick="addOption({{$n}})"> إضافة خيار </button>
                </div>
                <?php $n++; } } ?>
              </div>
                <div class="row mt-1 ">
...
<div class="card-header">
  ...
  <div class="card-title">
    <h3 class="card-label">{{trans(lang_app_site().'.CP.Product Images')}}</h3>
                  ...
                  <div class="col-md-12">
                      <div class="portlet light ">
                          ...
                          <div class="portlet-body form">
                              ...
                              {!! Form::open(['method'=>'POST','url'=>url($EditView),'id'=>'fileuploadProduct','files'=>true]) !!}
                              <!-- The fileupload-buttonbar contains buttons to add/delete files and start/cancel the upload -->
                                  ...
                                  <div class="fileupload-buttonbar">
                                    ...
                                    <span class="btn btn-circle btn-primary fileinput-button">
                                      <i class="fa fa-plus"></i>
                                      <span>{{trans(lang_app_site().'.CP.Add files')}}</span>
                                      <input type="file" name="files[]" multiple="">
                                    </span>
                                    <button type="reset" class="btn btn-circle btn-danger cancel">
                                        <i class="fa fa-ban-circle"></i>
                                        <span>{{trans(lang_app_site().'.CP.Cancel upload')}}</span>
                                    </button>
                                  </div>
            ...
            <tr class="template-upload ">
                ...
                <td> {% if (!i && !o.options.autoUpload) { %}
                    <button class="btn btn-info start" disabled>
                        ...
                        <span>{{trans(lang_app_site().'.CP.Start')}}</span>
                    </button> {% } %} {% if (!i) { %}
                    <button class="btn btn-danger cancel">
                        <i class="fa fa-ban"></i>
                        <span>{{trans(lang_app_site().'.CP.Cancel')}}</span>
                    </button> {% } %} </td>
            </tr> {% } %}
</script>
            ...
            <tr class="template-download">
                ...
                <td> {% if (file.deleteUrl) { %}
                    <button class="btn btn-danger {%=file.deleteClass%} btn-sm" data-type="{%=file.deleteType%}" data-url="{%=file.deleteUrl%}" {% if (file.deleteWithCredentials) { %} data-xhr-fields='{"withCredentials":true}' {% } %}>
                        ...
                        <span>{{trans(lang_app_site().'.CP.Delete')}}</span>
                    </button>
                     {% } else { %}
