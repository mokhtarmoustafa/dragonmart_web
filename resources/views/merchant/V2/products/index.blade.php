...
<div class="card card-custom">
    ...
    <div class="card-header">
        ...
        <div class="card-toolbar">
            <a href="{{url(merchant_url().'/product/create')}}" class="btn btn-circle btn-primary add-category-mdl">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_products">
                    <thead>
                        <tr role="row" class="heading">
                            ...
                            <th width="10%">{{trans(lang_app_site().'.CP.Offer')}} </th>
                            <th width="10%">{{trans(lang_app_site().'.CP.Sponsor')}} </th>
                            {{--<th width="10%">{{trans(lang_app_site().'.CP.Price')}} </th>--}}
                            {{--<th width="10%">{{trans(lang_app_site().'.CP.Quantity')}} </th>--}}
                            <th width="10%">{{trans(lang_app_site().'.CP.Category')}} </th>
                            <th width="10%">{{trans(lang_app_site().'.CP.Action')}} </th>
                        </tr>
                        <tr role="row" class="filter">
                            <td></td>
                            <td>
                                <input type="text" class="form-control form-filter input-md" name="name" placeholder="{{trans(lang_app_site().'.CP.Name')}}" id="product_name">
                            </td>

                            <td>
                                <select class="form-control input-md is_offer" name="is_offer" id="is_offer" data-placeholder="Choose Offer Status">
                                    <option value="">{{trans(lang_app_site().'.CP.Offer')}}</option>
                                    <option value="0">{{trans(lang_app_site().'.CP.No')}}</option>
                                    <option value="1">{{trans(lang_app_site().'.CP.Yes')}}</option>
                                </select>
                            </td>

                            <td>
                                <select class="form-control input-md is_sponsor" name="is_sponsor" id="is_sponsor" data-placeholder="Choose Sponsor Status">
                                    <option value="">{{trans(lang_app_site().'.CP.Sponsor')}}</option>
                                    <option value="0">{{trans(lang_app_site().'.CP.No')}}</option>
                                    <option value="1">{{trans(lang_app_site().'.CP.Yes')}}</option>
                                </select>
                            </td>
                            <td>
                                <select class="form-control input-md category_id" name="category_id" id="category_id" data-placeholder="Choose Category">
                                    <option value="">{{trans(lang_app_site().'.CP.Category')}}</option>
        ...
        <table class="table table-hover table-checkable order-column" id="products_tbl">
            <thead>
                <tr>
                    ...
                    <th> {{trans(lang_app_site().'.CP.Sponsor')}}</th>
                    <th> {{trans(lang_app_site().'.CP.Offer')}}</th>
                    <th> {{trans(lang_app_site().'.CP.Offer')}} %</th>
