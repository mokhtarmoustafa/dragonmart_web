...
<div class="accordion accordion-solid accordion-panel accordion-svg-toggle" id="stores">
  ...
  <?php foreach ($store as $store_id => $storeInfo) : ?>
    ...
    <div class="card">
      <div class="card-header" id="heading{{$storeInfo['id']}}">
        <div class="card-title collapsed" data-toggle="collapse" data-target="#store{{$storeInfo['id']}}">
          <div class="card-label">{{$storeInfo['name']}}</div>
          <span class="svg-icon">
      ...
      <div id="store{{$storeInfo['id']}}" class="collapse" data-parent="#stores">
        <div class="card-body">
            ...
            <div class="card-header card-header-tabs-line">
              <div class="card-toolbar">
                <ul class="nav nav-tabs nav-bold nav-tabs-line">
                  <li class="nav-item">
                    <a class="nav-link active" data-toggle="tab" href="#tab_general_{{$storeInfo['id']}}">
                      <span class="nav-icon"><i class="flaticon2-chat-1"></i></span>
                      <span class="nav-text">المعلومات الاساسية</span>
                    </a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" data-toggle="tab" href="#tab_times_{{$storeInfo['id']}}">
            ...
            <div class="card-body">
                  <form class="" action="{{url('merchant/store/'.$storeInfo['id'])}}" method="post" id="store_{{$storeInfo['id']}}">
              <div class="tab-content">
                <div class="tab-pane fade show active" id="tab_general_{{$storeInfo['id']}}" role="tabpanel" aria-labelledby="tab_general_{{$storeInfo['id']}}">

                    @csrf
                    <div class="form-group">
                      <div class="form-group row pt-3 pr-5 pl-5">
                        <div class="col-md-6">
                          <label>{{trans(lang_app_site().'.CP.Store name')}}</label>
                          <input id="name" class="form-control" name="name" type="text" placeholder="{{trans(lang_app_site().'.CP.Store name')}}" value="{{$storeInfo['name'] ?? ''}}" required>
                        </div>

                        <div class="col-md-6">
                          <label>{{trans(lang_app_site().'.CP.Store Phone')}}</label>
                          <div class="input-group">
                            <span class="input-group-addon mt-auto mb-auto mr-2" id="basic-addon1">+966</span>
                            <input id="phone" class="form-control" name="phone" type="number" placeholder="{{trans(lang_app_site().'.CP.Store Phone')}}" value="{{$storeInfo['phone'] ?? ''}}" min="500000000" max="5999999999" required>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="control-label col-md-2">
                        <label>{{trans(lang_app_site().'.CP.Map')}}</label>
                      </div>
                      <div class="col-md-12">
                        <input type="hidden" name="lat" value="{{$storeInfo['lat']}}" id="latmap{{$storeInfo['id']}}">
                        <input type="hidden" name="lng" value="{{$storeInfo['lng']}}" id="lngmap{{$storeInfo['id']}}">

                        <div id="map{{$storeInfo['id']}}" style="height: 300px;"></div>
                      </div>
                    </div>


                </div>
                <div class="tab-pane fade" id="tab_times_{{$storeInfo['id']}}" role="tabpanel" aria-labelledby="tab_times_{{$storeInfo['id']}}">
                  <table class="table table-bordered mt-5">
                    ...
                    <tbody>
                      ...
                      @endphp 
                      @foreach(config('app.days_names') as $day)
                      ...
                      <tr>
                        ...
                        <td>
                        <input type="time" name="time[open][{{$day}}]" class="form-control" value="{{$storeInfo['open_times'][$day]}}" />
                        </td>
                        <td>
                        <input type="time" name="time[close][{{$day}}]" class="form-control" value="{{$storeInfo['close_times'][$day]}}" />
                        </td>
                        <td>
                        <select class="form-control"  name="time[state][{{$day}}]">
                          <option value="1" @if($storeInfo['state_times'][$day] == 1) selected @endif>تفعيل</option>
                          <option value="0" @if($storeInfo['state_times'][$day] == 0) selected @endif>تعطيل</option>
                        </select>
                        </td>
                      </tr>
                ...
                @if(is_null($storeInfo['deleted_at']))
                      <a href="{{url('merchant/store/'.$storeInfo['id'].'/delete')}}" class="btn btn-circle btn-icon-only red" >
                        <i class="fa fa-trash"></i>
                      </a>


                        <button class="btn btn-circle btn-info add-category-mdl" form="store_{{$storeInfo['id']}}">
                            <i class="fa fa-check"></i>
                            <span class="hidden-xs"> {{trans(lang_app_site().'.CP.save')}}</span>
                        </button>
                        @else
                        <a href="{{url('merchant/store/'.$storeInfo['id'].'/recover')}}" class="btn btn-circle btn-icon-only blue" >
    ...
    <div class="card card-custom">
      <div class="card-header">
        <div class="card-title">
          ...
          <span class="caption-subject bold uppercase font-blue-dark"> {{trans(lang_app_site().'.CP.New Store')}}</span>
        </div>
        <div class="card-toolbar">
      ...
      <div class="portlet-body">
        <form class="" action="{{url('merchant/store/new')}}" method="post" id="NewStore">
          ...
          <div class="form-group">
            <div class="form-group row pt-3 pr-5 pl-5">
              <div class="col-md-6">
                <label>{{trans(lang_app_site().'.CP.Store name')}}</label>
                <input id="name" class="form-control" name="name" type="text" placeholder="{{trans(lang_app_site().'.CP.Store name')}}" value="" required>
              </div>
              ...
              <div class="col-md-6">
                ...
                <div class="input-group">
                  <span class="input-group-addon mt-auto mb-auto mr-2" id="basic-addon1">+966</span>
                  <input id="phone" class="form-control" name="phone" type="number" placeholder="{{trans(lang_app_site().'.CP.Store Phone')}}" value="" min="500000000" max="5999999999" required>
                </div>
              </div>
            </div>
...
<script>
  @foreach($store as $store_id => $storeInfo)
  var map_{{$storeInfo['id']}};
  @endforeach
  ...
  @foreach($store as $store_id => $storeInfo)
  myMap2("map{{$storeInfo['id']}}", "ssss" @if($storeInfo['lat'] != "") {{','.$storeInfo['lat'] }} {{','.$storeInfo['lng']}} @endif);
  @endforeach
