        ...
        <div class="portlet box purple">
            ...
            <div class="portlet-body form">
                <div class="form-body">
                    ...
                    {!! Form::open(['method'=>'PUT','class'=>'form-horizontal form-bordered form-row-stripped','url'=>url(merchant_store_url().'/product/'.$product->id),'id'=>'productEdit']) !!}
                    ...
                    <div class="form-group">
                        ...
                        <div class="control-label col-md-2 @if(!$product->is_offer) has_offer @endif">
                            <label>Offer %</label>
                        </div>
                        <div class="col-md-2 @if(!$product->is_offer) has_offer @endif">
                            <input id="offer_percentage" class="form-control" name="offer_percentage" type="number"
                                   placeholder="Offer %" value="{{$product->offer_percentage ?? null}}">
                        </div>
                        ...
                        <div class="col-md-1">
                            <div class="md-checkbox"><input type="checkbox" id="checkbox2" name="is_sponsor"
                                                            @if($product->is_sponsor) checked @endif
                            ...
                            <table class="table table-striped table-bordered table-hover table-checkable">
                                ...
                                @if(count($product->Customizations) > 0)
                                    ...
                                    @foreach($product->Customizations as $custom)
                                        @foreach($custom->product_customizations as $product_custom)
                                            <tr>
                                                <td>{{$loop->iteration}}</td>
                                                <td>{{$custom->name}}<input type="hidden" name="custom_id[]"
                                                                            value="{{$custom->id}}"></td>
                                                <td>{{$product_custom->price}}<input type="hidden" name="custom_price[]"
                                                                                    value="{{$custom->pivot->price}}">
                                                </td>
                                                <td>{{$product_custom->text}}<input type="hidden" name="custom_text[]"
                                                                                     value="{{$custom->pivot->text}}">
                                                </td>
                                                <td>{{$product_custom->description}}<input type="hidden"
                                                                                          name="custom_description[]"
                                                                                          value="{{$product_custom->description}}">
                                                </td>
                                                <td>
                                                    <span class="default_text">{{($product_custom->is_default)?'Yes':'No'}}</span><input
                                                            type="hidden" name="is_default[]" class="default"
                                                            value="{{($product_custom->is_default)}}"></td>
                                                <td><a href="javascript:;"
