        ...
        <div class="container">
            <div class="breadcrumbs">
                <a href="{{url('site/home')}}">{{trans('app.site.home.home')}}</a> \ <span
                        class="current">{{trans('app.shopping_cart')}}</span>
            </div>
            <div class="row content-form">
                            ...
                            <td colspan="6" class="left">
                                <div class="left">
                                    <a href="{{url(site_url().'/home')}}"
                                       class="continue-shopping submit">{{trans('app.continue_shopping')}}</a>
                                </div>
                                <div class="right">
                                    <a href="javascript:;" onclick="removeAllFromCat()"
                                       class="submit update clear_cart">{{trans('app.site.product.clear_cart')}}</a>
                                    <a href="{{url(site_url().'/shipping-cart')}}"
                                       class="submit update">{{trans('app.update_shopping_cart')}}</a>
                                </div>
                            </td>
                        </tr>
                    </table>
                ...
                <div class="col-xs-12 col-sm-12 col-md-4 col-lg-3 ">
                    <div class="proceed-checkout">
                        <h4 class="main-title">{{trans('app.continue_checkout')}}</h4>
                        <div class="content">
                            <h5 class="title">{{trans('app.site.product.total_price')}}</h5>
                            <div class="info-checkout">
                                <span class="text">{{trans('app.cart_items')}}: </span><span class="item"
                                                                                             id="countitemds">0</span>
                            </div>
                            <div class="info-checkout">
                                <span class="text">{{trans('app.site.product.cart_subtotal')}}: </span><span
                                        class="item totalprice">0 {{trans('app.site.home.sar')}}</span>
                            </div>
                            <div class="total-checkout">
                                <span class="text">{{trans('app.site.product.total_price')}} </span><span
                                        class="price totalprice"> 0 {{trans('app.site.home.sar')}}</span>
                            </div>
                            <div class="group-button">
                                <a href="{{url(site_url().'/order')}}"
                                   class="button submit">{{trans('app.site.product.checkout')}}</a>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('javascript')
    <script>
                ...
                jQuery.ajax({
                    ...
                    success: function (data) {
                        if (data.items.length > 0) {
                            ...
                            window.location = '{{url(site_url()).'/home'}}';
                        }
                    }
                });
        ...
        function getCartToTable() {
            ...
            if (typeof (cartfromstorage) != "undefined" && cartfromstorage !== null) {
                ...
                if (typeof (cart) != "undefined" && cart !== null) {
                    ...
                    item += ' <h4 class="shop-title">{{trans('app.cart_shipping')}}</h4>';
                    item += ' <table class="shopping-cart-content" >';
                    item += '<tr class="title">';
                    item += '  <td width="80" class="product-thumb"></td>';
                    item += '  <td class="product-name">{{ucfirst(trans(lang_app_site().'.product.name'))}}</td>';
                    item += '<td class="price">{{ucfirst(trans(lang_app_site().'.product.unit_price'))}}</td>';
                    item += ' <td class="quantity-item">{{ucfirst(trans(lang_app_site().'.product.quantity'))}}</td>';
                    item += '  <td class="total">{{ucfirst(trans(lang_app_site().'.product.cart_subtotal'))}}</td>';
                    ...
                    for (var i = 0; i < cart.cart.length; i++) {
                        ...
                        item += ' <td class="product-thumb"><a href="{{url(site_url().'/product-page')}}/' + cart.cart[i].id + '"> ';
                        item += '<img src="' + valimage + '" alt=""></a></td>';
                        item += '<td class="product-name" data-title="Product Name"><a href="{{url(site_url().'/product-page')}}/' + cart.cart[i].id + '" class="product-name">Until ';
                        ...
                        item += ' </div>';
                        item += '   </td> ';
                        item += '<td class="total" data-title="SubTotal">' + cart.cart[i].totalprice + ' {{trans(lang_app_site().'.home.sar')}}</td>';
                    ...
                    $('.totalprice').html(totalprice_ + ' {{trans(lang_app_site().'.home.sar')}}');
        ...
        function removeFromCat(id) {
            ...
            toastTitle = '{{trans(lang_app_site().'.product.remove_item')}}';
        ...
        function proccessCartTable(data) {
            ...
            for (var j = 0; j < newcart.length; j++) {
                ...
                item += '<tr class="title">';
                item += '  <td width="80" class="product-thumb"></td>';
                item += '  <td class="product-name">{{ucfirst(trans('app.site.product.name'))}}</td>';
                item += '<td class="price">{{ucfirst(trans('app.site.product.unit_price'))}}</td>';
                item += ' <td class="quantity-item">{{ucfirst(trans('app.site.product.quantity'))}}</td>';
                item += '  <td class="total">{{ucfirst(trans('app.site.product.cart_subtotal'))}} {{trans(lang_app_site().'.home.sar')}}</td>';
                item += '   <td class="delete-item"></td>';
                item += '</tr>';
                ...
                for (var i = 0; i < cart.length; i++) {
                    ...
                    item += ' <tr class="each-item">';
                    item += ' <td class="product-thumb"><a href="{{url(site_url().'/product-page')}}/' + cart[i].product.id + '"> ';
                    ...
                    item += '<img src="' + valimage + '" alt=""></a></td>';
                    item += '<td class="product-name" data-title="Product Name"><a href="{{url(site_url().'/product-page')}}/' + cart[i].product.id + '" class="product-name">Until ';
                    ...
                    item += '<td class="total" data-title="SubTotal">' + totalprice_prod + ' {{trans('app.site.home.sar')}}</td>';
            ...
            $('.totalprice').html(total + ' {{trans('app.site.home.sar')}}');
