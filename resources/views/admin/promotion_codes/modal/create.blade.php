<div class="modal-content" id="modal-content">
	<div class="modal-header">
		<h3 class="modal-title" id="myModalLabel1">{{trans(lang_app_site().'.CP.New Promotion Code')}}</h3>
		<button type="button" class="close rounded-pill" data-dismiss="modal" aria-label="Close">
			<i class="bx bx-x"></i>
		</button>
	</div>
	<div class="modal-body">
		<form action="{{url()->current()}}" id="promotion_code" method="post">
			{{ csrf_field() }}
			<div class="row mt-1">
				<div class="col-6">
					<!-- الكود -->
									...
									<div class="row">
										<div class="col ml-0">
											<div class="input-group date" id="kt_datetimepicker_7_1" data-target-input="nearest">
												<input type="text" class="form-control datetimepicker-input" name="action[start_date]" placeholder="{{trans(lang_app_site().'.CP.From')}}" data-target="#kt_datetimepicker_7_1" />
										...
										<div class="col">
											<div class="input-group date" id="kt_datetimepicker_7_2" data-target-input="nearest">
												<input type="text" class="form-control datetimepicker-input" name="action[end_date]" placeholder="{{trans(lang_app_site().'.CP.To')}}" data-target="#kt_datetimepicker_7_2" />
						...
						<div class="input-group ">
							<div class="input-group-prepend">
								<select class="form-control rounded-right-0" name="action[amount_type]" id="amount_type">
									<option value="static">{{trans(lang_app_site().'.CP.Static Amount')}}</option>
									<option value="percentage">{{trans(lang_app_site().'.CP.Percentage')}}</option>
								</select>
					...
					<div class="max_discount col-md-12 col-12 hidden d-none" id="max_discount">
						<div class="form-group w-100">
							<div class="form-group">
								<input type="text" class="form-control" name="action[max_discount]" placeholder="{{trans(lang_app_site().'.CP.Discount To')}}" disabled required>
							</div>
						</div>
					</div>
				</div>
				<div class="col-6">
					...
					<div class="col-md-12 col-12">
						<div class="form-group">
							...
							<select class="form-control" name="action[promo_on]" id="promo_on">
								<option value="delivery">{{trans(lang_app_site().'.CP.Delivery')}}</option>
								<option value="products">{{trans(lang_app_site().'.CP.Products')}}</option>
								<option value="delivery_products">{{trans(lang_app_site().'.CP.Delivery and Products')}}</option>
							</select>
						</div>
					</div>
					<div class="promo_on col-md-12 col-12 hidden d-none" id="products_promo">
						<div class="form-group">
							<select class="form-control" name="action[products_promo]" id="select_products_promo">
								<option value="all_stores">{{trans(lang_app_site().'.CP.All Stores and Categories')}}</option>
								<option value="category">{{trans(lang_app_site().'.CP.Category')}}</option>
					...
					<div class="condition_minimum col-md-12 col-12 hidden d-none" id="condition_minimum">
						<div class="form-group w-100">
							<div class="form-group">
								<input type="text" class="form-control" name="action[condition_minimum]" placeholder="{{trans(lang_app_site().'.CP.Minimum')}}" disabled required>
							</div>
						</div>
					</div>
					<div class="condition_maximum col-md-12 col-12 hidden d-none" id="condition_maximum">
						<div class="form-group w-100">
							<div class="form-group">
								<input type="text" class="form-control" name="action[condition_maximum]" placeholder="{{trans(lang_app_site().'.CP.Maximum')}}" disabled required>
							</div>
						</div>
					</div>
