$(document).ready(function () {
    ...
    if ($("#shipment_tbl").length) {

        var constant = $('#constant').val();
        var shipment_tbl = $("#shipment_tbl");
        shipment_tbl.on('preXhr.dt', function (e, settings, data) {
        ...
        $(".dataTables_filter input")
            ...
            .bind("input", function (e) { // Bind our desired behavior
                ...
                if (this.value.length >= 3 || e.keyCode == 13) {
                    ...
                    shipment_tbl.api().search(this.value).ajax.reload();

                }
                // Ensure we clear the search if they backspace far enough
                if (this.value == "") {
                    shipment_tbl.api().search("").ajax.reload();
                }
            });
    }
    ...
    $(document).on('click', '.edit', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status) {
                    $('#shipment_name').val(data.items.name);
                    $('#save_shipment_frm').attr('action', action);
...
//                if ($(this).val().length > 3)
    ...
    $(document).on('submit', '#save_shipment_frm,#editProfile', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status) {
                    ...
                    if (event.target.id == 'save_shipment_frm') {
                        shipment_tbl.api().ajax.reload();
                        // $('#save_shipment_frm').attr('action', baseURL + '/constant/shipment');
                        //
                        // $('#icon').val('');
                        // $('#shipment_name').val('');
                    }
